    ...
    <div  class="pagecont-bottom">
      <div class="justify-between mb15">
        <el-row :gutter="15" class="btn-style">
          <el-col :span="1.5">
            <el-button v-ripple class="glass-btn" plain @click="handleAdd" v-hasPermi="['dm:dmExpertAdvice:advice:add']"
                       @mousedown="(e) => e.preventDefault()">
              <i class="iconfont-mini icon-xinzeng mr5"></i>新增
            </el-button>
          </el-col>
<!--          <el-col :span="1.5">-->
<!--            <el-button v-ripple class="glass-btn" plain :disabled="single" @click="handleUpdate" v-hasPermi="['dm:dmExpertAdvice:advice:edit']"-->
<!--                       @mousedown="(e) => e.preventDefault()">-->
<!--              <i class="iconfont-mini icon-xiugai&#45;&#45;copy mr5"></i>修改-->
<!--            </el-button>-->
<!--          </el-col>-->
<!--          <el-col :span="1.5">-->
<!--            <el-button v-ripple class="glass-btn" plain :disabled="multiple" @click="handleDelete" v-hasPermi="['dm:dmExpertAdvice:advice:remove']"-->
<!--                       @mousedown="(e) => e.preventDefault()">-->
<!--              <i class="iconfont-mini icon-shanchu-huise mr5"></i>删除-->
<!--            </el-button>-->
<!--          </el-col>-->
<!--          <el-col :span="1.5">-->
<!--            <el-button v-ripple class="glass-btn" plain  @click="handleImport" v-hasPermi="['dm:dmExpertAdvice:advice:export']"-->
<!--                       @mousedown="(e) => e.preventDefault()">-->
<!--              <i class="iconfont-mini icon-upload-cloud-line mr5"></i>导入-->
<!--            </el-button>-->
<!--          </el-col>-->
<!--          <el-col :span="1.5">-->
<!--            <el-button v-ripple class="glass-btn" plain @click="handleExport" v-hasPermi="['dm:dmExpertAdvice:advice:export']"-->
<!--                       @mousedown="(e) => e.preventDefault()">-->
...
<!--          </el-col>-->
      ...
      <el-table class="glass-card" stripe  v-loading="loading" :data="adviceList" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
...
<!--        <el-table-column v-if="getColumnVisibility(3)" label="工况编码" align="center" prop="condCode">-->
<!--          <template #default="scope">-->
<!--            {{ scope.row.condCode || '-' }}-->
<!--          </template>-->
<!--        </el-table-column>-->
        <el-table-column v-if="getColumnVisibility(4)" label="工况" align="center" prop="operateCond">
          <template #default="scope">
        ...
        <el-table-column v-if="getColumnVisibility(6)" label="告警条件" align="center" prop="alarmCond">
          <template #default="scope">
            {{ scope.row.alarmCond || '-' }}
          </template>
        </el-table-column>
        <el-table-column v-if="getColumnVisibility(7)" label="告警内容" align="center" prop="content">
          <template #default="scope">
            {{ scope.row.content || '-' }}
          </template>
        </el-table-column>
        <el-table-column v-if="getColumnVisibility(8)" label="预案" align="center" prop="plan">
          <template #default="scope">
            {{ scope.row.plan || '-' }}
          </template>
        </el-table-column>
        <el-table-column v-if="getColumnVisibility(9)" label="预案简称" align="center" prop="planShort">
          <template #default="scope">
            {{ scope.row.planShort || '-' }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="240">
          <template #default="scope">
            <el-button v-ripple class="glass-btn" link icon="Edit" @click="handleUpdate(scope.row)"
                       v-hasPermi="['dm:dmExpertAdvice:advice:edit']">修改</el-button>
            <el-button v-ripple class="glass-btn" link icon="Delete" @click="handleDelete(scope.row)"
                       v-hasPermi="['dm:dmExpertAdvice:advice:remove']">删除</el-button>
            <el-button v-ripple class="glass-btn" link icon="view" @click="handleDetail(scope.row)"
                       v-hasPermi="['dm:dmExpertAdvice:advice:edit']">详情</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="title" v-model="open" width="800px" append-to="body" draggable class="glass-card">
      ...
      <el-form ref="adviceRef" :model="form" :rules="rules" label-width="80px" @submit.prevent>
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="告警名称" prop="name">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="告警条件" prop="alarmCond">
              <el-input v-model="form.alarmCond" type="textarea" placeholder="请输入内容" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="告警内容" prop="content">
              <el-input v-model="form.content" type="textarea" placeholder="请输入告警内容" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
    ...
    <el-dialog :title="title" v-model="openDetail" width="800px" append-to="body" draggable class="glass-card">
      ...
      <el-form ref="adviceRef" :model="form"  label-width="80px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="告警名称" prop="name">
              <div>
        ...
        <template #tip>
          <div class="el-upload__tip text-center">
            <div class="el-upload__tip">
              <el-checkbox v-model="upload.updateSupport" />是否更新已经存在的专家经验数据
            </div>
...
const columns = ref([
  ...
  { key: 6, label: "告警条件", visible: true },
  { key: 7, label: "告警内容", visible: true },
  { key: 8, label: "预案", visible: true },
  { key: 9, label: "预案简称", visible: true },
...
const upload = reactive({
  ...
  // 上传的地址
  url: import.meta.env.VITE_APP_BASE_API + "/dm/advice/importData"
});
...
function getList() {
  ...
  listAdvice(queryParams.value).then(response => {
    adviceList.value = response.data.rows;
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["adviceRef"].validate(valid => {
    if (valid) {
      ...
      if (form.value.id != null) {
        updateAdvice(form.value).then(response => {
        ...
        addAdvice(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除专家经验编号为"' + _ids + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('dm/advice/export', {
    ...queryParams.value
...
function importTemplate() {
  ...
  }, `advice_template_${new Date().getTime()}.xlsx`)
}

<template>
  <div class="app-container glass-card" ref="app-container">
    ...
    <div  class="pagecont-bottom">
     <div class="justify-between mb15">
       <el-row :gutter="15" class="btn-style">
         <el-col :span="1.5">
           <el-button v-ripple class="glass-btn" plain @click="handleAdd" v-hasPermi="['ext:extUnstructTaskDocRel:unstructtaskdocrel:add']"
                      @mousedown="(e) => e.preventDefault()">
             <i class="iconfont-mini icon-xinzeng mr5"></i>新增
           </el-button>
         </el-col>
         <el-col :span="1.5">
           <el-button v-ripple class="glass-btn" plain :disabled="single" @click="handleUpdate" v-hasPermi="['ext:extUnstructTaskDocRel:unstructtaskdocrel:edit']"
                      @mousedown="(e) => e.preventDefault()">
             <i class="iconfont-mini icon-xiugai--copy mr5"></i>修改
           </el-button>
         </el-col>
         <el-col :span="1.5">
           <el-button v-ripple class="glass-btn" plain :disabled="multiple" @click="handleDelete" v-hasPermi="['ext:extUnstructTaskDocRel:unstructtaskdocrel:remove']"
                      @mousedown="(e) => e.preventDefault()">
             <i class="iconfont-mini icon-shanchu-huise mr5"></i>删除
           </el-button>
         </el-col>
         <el-col :span="1.5">
           <el-button v-ripple class="glass-btn" plain  @click="handleImport" v-hasPermi="['ext:extUnstructTaskDocRel:unstructtaskdocrel:export']"
                      @mousedown="(e) => e.preventDefault()">
             <i class="iconfont-mini icon-upload-cloud-line mr5"></i>导入
           </el-button>
         </el-col>
         <el-col :span="1.5">
           <el-button v-ripple class="glass-btn" plain @click="handleExport" v-hasPermi="['ext:extUnstructTaskDocRel:unstructtaskdocrel:export']"
                      @mousedown="(e) => e.preventDefault()">
     ...
     <el-table class="glass-card" stripe  v-loading="loading" :data="unstructTaskDocRelList" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
       ...
       <el-table-column v-if="getColumnVisibility(3)" label="文件id" align="center" prop="docId">
         <template #default="scope">
           {{ scope.row.docId || '-' }}
         </template>
       </el-table-column>
       <el-table-column v-if="getColumnVisibility(4)" label="文件名" align="center" prop="docName">
         <template #default="scope">
           {{ scope.row.docName || '-' }}
         </template>
       </el-table-column>
       ...
       <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="240">
         <template #default="scope">
           <el-button v-ripple class="glass-btn" link icon="Edit" @click="handleUpdate(scope.row)"
                      v-hasPermi="['ext:extUnstructTaskDocRel:unstructtaskdocrel:edit']">修改</el-button>
           <el-button v-ripple class="glass-btn" link icon="Delete" @click="handleDelete(scope.row)"
                      v-hasPermi="['ext:extUnstructTaskDocRel:unstructtaskdocrel:remove']">删除</el-button>
           <el-button v-ripple class="glass-btn" link icon="view" @click="handleDetail(scope.row)"
                      v-hasPermi="['ext:extUnstructTaskDocRel:unstructtaskdocrel:edit']">详情</el-button>
           <el-button v-ripple class="glass-btn" link icon="view" @click="routeTo('/ext/extUnstructTaskDocRel/unstructTaskDocRelDetail',scope.row)"
                      v-hasPermi="['ext:extUnstructTaskDocRel:unstructtaskdocrel:edit']">复杂详情</el-button>
         </template>
       </el-table-column>
    ...
    <el-dialog class="glass-card" :title="title" v-model="open" width="800px" append-to="body" draggable>
      ...
      <el-form ref="unstructTaskDocRelRef" :model="form" :rules="rules" label-width="80px" @submit.prevent>
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="工作区id" prop="workspaceId">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12">
              <el-form-item label="文件名" prop="docName">
                <el-input v-model="form.docName" placeholder="请输入文件名" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
    ...
    <el-dialog class="glass-card" :title="title" v-model="openDetail" width="800px" append-to="body" draggable>
      ...
      <el-form ref="unstructTaskDocRelRef" :model="form"  label-width="80px">
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="工作区id" prop="workspaceId">
                <div>
    ...
    <!-- 用户导入对话框 -->
    <el-dialog class="glass-card" :title="upload.title" v-model="upload.open" width="800px"  append-to="body" draggable destroy-on-close>
      <el-upload
          ref="uploadRef"
          :limit="1"
          accept=".xlsx, .xls"
        ...
        <template #tip>
          <div class="el-upload__tip text-center">
            <div class="el-upload__tip">
              <el-checkbox v-model="upload.updateSupport" />是否更新已经存在的任务文件关联数据
            </div>
...
<script setup name="UnstructTaskDocRel">
  import { listUnstructTaskDocRel, getUnstructTaskDocRel, delUnstructTaskDocRel, addUnstructTaskDocRel, updateUnstructTaskDocRel } from "@/api/ext/extUnstructTaskDocRel/unstructTaskDocRel";
  ...
  const upload = reactive({
    ...
    // 上传的地址
    url: import.meta.env.VITE_APP_BASE_API + "/ext/unstructTaskDocRel/importData"
  });
  ...
  const data = reactive({
    ...
    rules: {
        ...
        docName: [{ required: true, message: "文件名不能为空", trigger: "blur" }],
  ...
  function getList() {
    loading.value = true;
    listUnstructTaskDocRel(queryParams.value).then(response => {
            unstructTaskDocRelList.value = response.data.rows;
  ...
  function reset() {
    ...
    proxy.resetForm("unstructTaskDocRelRef");
  }
  ...
  function handleDetail(row) {
    ...
    getUnstructTaskDocRel(_id).then(response => {
      ...
      title.value = "任务文件关联详情";
    });
  }

  /** 提交按钮 */
  function submitForm() {
    proxy.$refs["unstructTaskDocRelRef"].validate(valid => {
      if (valid) {
        if (form.value.id != null) {
          updateUnstructTaskDocRel(form.value).then(response => {
          ...
          addUnstructTaskDocRel(form.value).then(response => {
  ...
  function handleDelete(row) {
    ...
    proxy.$modal.confirm('是否确认删除任务文件关联编号为"' + _ids + '"的数据项？').then(function() {
      return delUnstructTaskDocRel(_ids);
    }).then(() => {
      getList();
  ...
  function handleExport() {
    proxy.download('ext/unstructTaskDocRel/export', {
      ...queryParams.value
    }, `unstructTaskDocRel_${new Date().getTime()}.xlsx`)
  }

  /** ---------------- 导入相关操作 -----------------**/
  /** 导入按钮操作 */
  function handleImport() {
    upload.title = "任务文件关联导入";
    upload.open = true;
  }
  ...
  function importTemplate() {
    ...
    }, `unstructTaskDocRel_template_${new Date().getTime()}.xlsx`)
  }

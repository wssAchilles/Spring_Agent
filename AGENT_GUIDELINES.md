...
| 9 | Git 提交已完成 | 阶段所有变更已提交，commit message 包含阶段 ID |

---

## 七、Git 提交规范

每个阶段完成并通过所有检查项后，必须执行 Git 提交。
...
```
feat(01-rag-knowledge-engine): RAG 知识引擎核心

- 实现文档上传接口 (PDF/Word/Markdown)
- 集成 Spring AI Embedding 实现文本向量化
- 实现 PgVector 向量存储与余弦相似度检索
- 实现 RAG 问答接口与 SSE 流式响应
- 新增依赖: spring-ai-pgvector-store
```

### 提交时机

- 阶段收尾的最后一步，在 log.md 和 log.json 均已生成并同步之后
- 提交前确认所有变更文件已暂存（`git add`）
- 提交后确认 `git status` 工作区干净
